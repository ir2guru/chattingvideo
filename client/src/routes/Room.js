...
const Room = (props) => {
    ...
    function MuteAudioStart(){
        setState(!state);
        const audiotrackss = userStream.current.getTracks().find(track => track.kind === 'audio');
    ...
    function EndMyCall(){
        csetState(!camstate);
        const videotrackss = userStream.current.getTracks().find(track => track.kind === 'video');
    ...
    return (
        ...
        <div  className="bg">
            ...
            <button className="btn" style={{backgroundColor: 'red'}} onClick={EndMyCall}><FontAwesomeIcon icon={faPhone} ></FontAwesomeIcon></button>
                 <button className="btn" style={{backgroundColor: state ? 'red' : 'green'}} onClick={MuteAudio}><FontAwesomeIcon icon={faMicrophone} ></FontAwesomeIcon></button>
                 <button className="btn" style={{backgroundColor: camstate ? 'red' : 'green'}} onClick={EnableVideo}><FontAwesomeIcon icon={faVideoCamera} ></FontAwesomeIcon></button>
                 <button className="btn" onClick={shareScreen}><FontAwesomeIcon icon={faDesktop} ></FontAwesomeIcon></button>
                 <p className="time">Call Duration: {minutes} : {seconds < 10 ? `0${seconds}` : seconds}</p>
